...
namespace JumaEngine
{
    ...
    bool Material::createMaterial(Shader* shader)
    {
        ...
        ShadersSubsystem* shadersSubsystem = engine->getSubsystem<ShadersSubsystem>();
        const JumaRE::MaterialParamsStorage& internalParams = shadersSubsystem->getEngineInternalParams();
        ...
        {
            shadersSubsystem->onEngineInternalParamChanged.bind(this, &Material::onEngineInternalParamChanged);
        }
        return true;
    }
    ...
    void Material::clearMaterial()
    {
        if (m_Material != nullptr)
        {
            ...
            engine->getSubsystem<ShadersSubsystem>()->onEngineInternalParamChanged.unbind(this, &Material::onEngineInternalParamChanged);
    ...
    bool Material::isEngineInternalMaterialParam(const jstringID& name) const
    {
        const Shader* shader = getBaseShader();
        return (shader != nullptr) && shader->getEngineInternalParamNames().contains(name);
    }
    void Material::onEngineInternalParamChanged(ShadersSubsystem* subsystem, const jstringID& internalParamName)
    {
        for (const auto& internalParam : getBaseShader()->getEngineInternalParamNames())
        {
            ...
            if (uniform != nullptr)
            {
                CopyMaterialParam(uniform->type, internalParam.key, m_Material, internalParamName, subsystem->getEngineInternalParams());
            }
            break;
        }
    }
